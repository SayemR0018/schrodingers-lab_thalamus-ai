...
import type { InterpolationValues, Translations } from "./types";
 ...
 * current store value → default 'en'. Falls back to English if a key is
 * missing in the active language so missing translations never crash UI.
 */
export function t(
  key: string,
  values?: InterpolationValues,
  ...
  // Fallback to English so the UI never breaks.
  if (activeLang !== "en") {
...
/**
 * React hook that subscribes to language changes in the app store so
 * components re-render when the user switches language.
 */
...
/**
 * Resolve the display text of a bilingual entity (e.g. SuggestedQuestion).
 * Returns the Bengali variant if active and present; otherwise the base.
 */
export function resolveBilingual<T extends { text: string; textBn?: string }>(
  item: T,
  language: Language
): string {
  if (language === "bn" && item.textBn) return item.textBn;
  return item.text;
}
